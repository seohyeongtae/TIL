...
public class SendAndReceiveSerialTCPIP implements SerialPortEventListener {
	...
	public SendAndReceiveSerialTCPIP(String portName, boolean mode) {
		try {
			if (mode == true) {
	...
	public void serialEvent(SerialPortEvent event) { // 데이터를 받을 때는 얘가 받는다.
		...
		case SerialPortEvent.DATA_AVAILABLE:
			...
			try {
				...
				ClientIoT client = new ClientIoT("192.168.0.9", 5555, "[Chaeguevara_IoT]");
				client.connect();
				client.sendMsg(ss);
	...
	public static void main(String args[]) throws IOException {
		...
		SendAndReceiveSerialTCPIP ss = new SendAndReceiveSerialTCPIP("COM3", true);
		ClientIoT client = new ClientIoT("192.168.0.9", 5555, "[Chaeguevara_IoT]");
		client.connect();
		while (true) {
